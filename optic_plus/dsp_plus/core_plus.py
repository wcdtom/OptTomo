...
def pulseShape_plus(pulseType, SpS=16, N=2048, alpha=0.6, Ts=1e-9):
    ...
    # for test
    fig, bx = plt.subplots()
    t_ = np.linspace(-Ts/2, Ts/2, N)
    bx.set(xlim=(-Ts/100, Ts/100), xlabel="t")
...
def sincFilterTaps(t, beta, Ts):
    ...
    f = np.linspace(-boundary[1], boundary[1], len(t))
    H_f = np.zeros(len(t), dtype=np.float64)

    for i, f_i in enumerate(f):
        if abs(f_i) <= boundary[0]:
            H_f[i] = (np.pi*f_i*Ts)/np.sin(np.pi*f_i*Ts)
        elif boundary[0] < abs(f_i) < boundary[1]:
            H_f[i] = ((np.pi*f_i*Ts)/np.sin(np.pi*f_i*Ts)) * np.cos((np.pi*Ts*abs(f_i))/(2*beta-boundary[0]))
        elif abs(f_i) >= boundary[1]:
            H_f[i] = 0
    # for test
    fig, ax = plt.subplots()
    _f = f*Ts
