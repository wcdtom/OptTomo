...
for sender_id in range(1, in_cast_degree+1):
    ...
    new_flow = Flow(fid=sender_id, src="flow " + str(sender_id), dst="flow " + str(sender_id), finish_time=100000,
                    arrival_dist=packet_arrival, size_dist=packet_size, size=flow_size, start_time=0.0001)
    new_sender = BBRPacketGenerator(env, flow=new_flow, cc=BBR(rtt_estimate=0.15), element_id=new_flow.src, debug=True)
